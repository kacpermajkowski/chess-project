...
struct TestSuiteKnightFixture {
    ...
    UnitPtr whiteKing = make_shared<King>(WHITE);
    UnitPtr blackKing = make_shared<King>(BLACK);
};
...
BOOST_FIXTURE_TEST_SUITE(TestSuiteKnight, TestSuiteKnightFixture)
    ...
    BOOST_AUTO_TEST_CASE(TestLegalMovesKnight){
        ...
        board->getField(make_shared<Position>(A, _1))->setUnit(whiteKing);
        board->getField(make_shared<Position>(A, _8))->setUnit(blackKing);
        ...
        list<PositionPtr> positions {
                std::make_shared<Position>(B, _7),
                std::make_shared<Position>(C, _6),
                std::make_shared<Position>(F, _7),
        };

        BOOST_TEST(positions.size() == moves.size());
        for(PositionPtr p: positions){
